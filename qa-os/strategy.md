...
describe('CreateProjectSchema', () => {
  it('accepts valid input', () => {
    const result = CreateProjectSchema.safeParse({ name: 'My Project', status: 'active' });
  ...
  it('rejects unknown status', () => {
    const result = CreateProjectSchema.safeParse({ name: 'Valid', status: 'unknown' });
    expect(result.success).toBe(false);
  });
});
```
...
test('tenant A cannot access tenant B projects', async ({ browser }) => {
  ...
  await signIn(pageA, process.env.TENANT_A_EMAIL!, process.env.TENANT_A_PASSWORD!);
  ...
  await signIn(pageB, process.env.TENANT_B_EMAIL!, process.env.TENANT_B_PASSWORD!);
