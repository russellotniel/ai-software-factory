...
Every Expo project in the Shannon organisation copies `.gunawan/` from this repo
...
| Area | Status | Notes |
|------|--------|-------|
| `.gunawan/` package | ✅ Complete | Self-contained Expo package — ready to copy to projects |
...
| Commands | ✅ Complete | 16 commands + roles commands in `.gunawan/.claude/commands/` |
| Agents | ✅ Complete | architect, builder, reviewer, explorer, devops, noahs-ark in `.gunawan/.claude/agents/` |
...
| MCP config | ✅ Complete | `.gunawan/.mcp.json` with 6 servers (Expo/NativeWind adapted) |
| Docs structure | ✅ Complete | knowledge/, plan/, implementation/ established |
| Project history | ✅ Complete | Previous GPT/Cursor work documented in reference/ |
...
3. **Gunawan (current)** — Full agentic framework established. `.gunawan/` package created and
   standardised for deployment to all Shannon Expo projects.
...
| `reference/mcp-setup.md` | MCP server setup guide — detailed per-server configuration (Expo-adapted) |
...
- [ ] Start an Expo project and validate the `.gunawan/` deployment process end-to-end
